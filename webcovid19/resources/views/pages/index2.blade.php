...
<!--Konten-->
<div class="p-4 mt-2" style="margin-top: 1cm;">
<!--Statistik-->
@if(session('success'))
...
<div class="p-3 rounded-lg shadow-lg" style="background: white; margin-top: 20px;">
  <div class="bg-dark">
<center><h1 style="color: white;">STATISTIK</h1></center>
</div>
<form action="{{ url('/dashboard2') }}">
  <br><a class="btn btn-primary" href="{{ url('/dashboard2/statistik') }}"> <img src="{{ asset('/img/icons8_add_property_64px.png') }}" height ="25px"> Tambah data </a>
...
<tr>
    <td>{{ isset($i) ? ++$i : $i = 1 }}</td>
    <td>{{ $row->Tanggal }}</td>
    <td style="color : Royalblue; font-weight: bold;">{{ $row->Pasien_positif }}</td>
    ...
    <td> <a href="{{ url('/dashboard2/' . $row->id . '/edit') }}"> <button class=" btn btn-warning"> <img src="{{ asset('/img/icons8_edit_64px.png') }}" height = "25px"> </button></a></td>
    <td>
        <form action="{{ url('/dashboard2', $row->id) }}" method="post">
          @method('DELETE')
          @csrf
          <button class=" btn btn-danger"> <img src="{{ asset('/img/icons8_erase_64px.png') }}" height = "25px"> </button>
        </form>
    </td>
</tr>
@endforeach
